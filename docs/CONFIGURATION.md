...
`ntt_adapter_get_default()` caches the resolved adapter until `ntt_adapter_set_default()` is called again or `ntt_adapter_unload_all()` clears the cache. `ntt_adapter_load(name, module_dir)` selects an adapter explicitly, bypassing the default resolution entirely: when `module_dir` is NULL it looks the name up in the built-in registry, and when a directory is given it goes straight to the module loader, which expects `<module_dir>/<prefix>ntt_adapter_<name><ext>` — the two are mutually exclusive, not a fallback chain.
